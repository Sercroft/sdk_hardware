...
import com.credibanco.sdk.util.BytesUtil.string2ASCIIByteArray
import com.credibanco.sdk.util.Constants.AMEX_FRANCHISE
import com.credibanco.sdk.util.Constants.EMPTY_STRING
import com.credibanco.sdk.util.Constants.FILE_SEPARATOR
import com.credibanco.sdk.util.Constants.MASTERCARD_FRANCHISE
import com.credibanco.sdk.util.Constants.SEPARATOR_DECIMAL
import com.credibanco.sdk.util.Constants.UNKNOWN_FRANCHISE
import com.credibanco.sdk.util.Constants.VISA_FRANCHISE
...
class EmvDataSourceIngenicoImpl @Inject constructor(
    private val context: Context,
): EmvDataSource, ServiceConnection, InitializationDataSource {
    companion object {
    ...
    private var systemTrace: String = EMPTY_STRING
    private var amountEmv = 0L
    private var amountEmvString = EMPTY_STRING
    ...
    private var pinBlock: String = EMPTY_STRING
    ...
    private suspend fun validateAndInitEmv() {
        ...
        if (changeTagForDcc) {
            if (totalAmount.toString().isNotEmpty()) {
                ...
                val amountSplit = amountRound.toString().split(SEPARATOR_DECIMAL)
    ...
    private fun validateFranchise(tag84String: String?): String {
        ...
        if (!tag84String.isNullOrEmpty()) {
            ...
            if (tag84InformationArray[0] == "A0" &&
                ...
                return AMEX_FRANCHISE
            } else {
                return UNKNOWN_FRANCHISE
            }
        }
    ...
    private fun onCardInfo(): Boolean{
        if (loadEmvTags()){
            ...
            emvCallback.invoke(EmvCallbackObjectSDK.Error(
                callBacktype = EmvCallbackObjectSDK.ERROR_CALLBACK
            ))
            return false
        }
    ...
    private fun register(useEpayModule: Boolean) {
        try {
            ...
            if(isKernelFunctionCall) kernelAvailableCallback.invoke(KernelAvailableStateCallBackObjectSDK.AvailableKernel(KernelAvailableStateCallBackObjectSDK.AVAILABLE_KERNEL))
    ...
    private fun getFrameToSend(serialNum: String): ByteArray {
        ...
        val lengthPlusSerialFrame = string2ASCIIByteArray(lengthPlusSerial)
